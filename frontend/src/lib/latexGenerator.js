...
function generatePreamble(globalSetup, metadata) {
    ...
    const preamble = [
        ...
        '  \\fancyhf{}',
        ...noHeaderLines.map(l => '  ' + l),
        '  \\renewcommand{\\headrulewidth}{0pt}',
        '}',
        '\\fancypagestyle{nofooter}{',
        '  \\fancyhf{}',
        ...noFooterLines.map(l => '  ' + l),
        '  \\renewcommand{\\headrulewidth}{0pt}',
        '}',
        '',
        '% ─── Hyperlinks & URLs ───────────────────────────────',
...
function blockToLatex(block, mirror = false, isFirst = false) {
    ...
    let { page_break, toc_visible = true, valign = 'top', hide_header = false, hide_footer = false } = config;
    ...
    // Configuração de visibilidade do cabeçalho/rodapé na página inicial do bloco
    if (hide_header && hide_footer) {
        tex += '\\thispagestyle{empty}\n';
    } else if (hide_header) {
        tex += '\\thispagestyle{noheader}\n';
    } else if (hide_footer) {
        tex += '\\thispagestyle{nofooter}\n';
    }
