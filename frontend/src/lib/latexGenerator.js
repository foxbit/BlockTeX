...
function mdToLatex(md) {
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith('```')) {
            ...
            i++; // fecha ```
            const langMap = { 'c++': 'C++', 'cpp': 'C++', 'js': 'JavaScript', 'javascript': 'JavaScript', 'python': 'Python', 'py': 'Python', 'java': 'Java', 'bash': 'bash', 'sh': 'bash', 'sql': 'SQL', 'html': 'HTML', 'css': 'CSS', 'json': 'JSON', 'xml': 'XML', 'c': 'C' };
            const latexLang = langMap[lang.toLowerCase()] || lang;
        ...
        if (hMatch) {
            ...
            // Adiciona ao TOC manualmente (pois usamos *)
...
function generatePreamble(globalSetup, metadata) {
    ...
    const fancyLines = mirror
        ? ['\\fancyfoot[LE,RO]{\\thepage}', '\\fancyhead[RE]{\\textit{\\leftmark}}', '\\fancyhead[LO]{\\textit{\\rightmark}}']
        : ['\\fancyfoot[C]{\\thepage}', '\\fancyhead[L]{\\textit{\\leftmark}}'];
